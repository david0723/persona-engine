...
| Path | Purpose |
|------|---------|
| `src/persona/schema.ts` | All TypeScript interfaces (PersonaDefinition, ContainerConfig, PermissionConfig) |
| `src/persona/defaults.ts` | Template YAML generated by `persona create` |
| `src/persona/loader.ts` | YAML parsing and validation |
| `src/runtime/opencode-config.ts` | Generates opencode.json (MCP servers + permissions) |
